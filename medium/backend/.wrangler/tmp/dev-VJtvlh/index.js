...
// .wrangler/tmp/bundle-hhbeuv/checked-fetch.js
  ...
  "node_modules/.prisma/client/edge.js"(exports) {
    ...
    var config = {
      "generator": {
        ...
        "output": {
          "value": "C:\\Users\\thisshonrobertsamuel\\Desktop\\PRJ2\\blogging-website\\medium\\backend\\node_modules\\@prisma\\client",
          "fromEnvVar": null
        },
        "config": {
          "engineType": "library"
...
// node_modules/@thisshon/medium-common/dist/index.js
var require_dist = __commonJS({
  "node_modules/@thisshon/medium-common/dist/index.js"(exports) {
    "use strict";
    init_checked_fetch();
    init_modules_watch_stub();
    var __importDefault = exports && exports.__importDefault || function(mod) {
...
// .wrangler/tmp/bundle-hhbeuv/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-hhbeuv/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
userRouter.post("/signup", async (c) => {
  ...
  try {
    ...
    const jwt = await sign2({ id: responsePayload.id }, jwtpassword);
    return c.json({
      jwt,
      name: body.name
    });
...
userRouter.post("/signin", async (c) => {
  ...
  const body = await c.req.json();
  const { success } = import_medium_common2.signinInput.safeParse(body);
  if (!success) {
    c.status(411);
    return c.json({
      error: "Invalid Inputs"
  ...
  try {
    ...
    const jwtpassword = c.env.JWT_SECRET;
    const jwt = await sign2({ id: existinguser.id }, jwtpassword);
    return c.json({
      jwt,
...
var import_medium_common3 = __toESM(require_dist());
var import_medium_common4 = __toESM(require_dist());
...
blogRouter.post("/", async (c) => {
  ...
  const body = await c.req.json();
  const { success } = import_medium_common4.createBlogInput.safeParse(body);
  if (!success) {
    c.status(411);
    return c.json({
      error: "Invalid Inputs"
...
blogRouter.put("/update", async (c) => {
  ...
  const body = await c.req.json();
  const userId = c.get("userId");
  const { success } = import_medium_common3.updateBlogInput.safeParse(body);
  if (!success) {
    c.status(411);
    return c.json({
      error: "Invalid Inputs"
...
blogRouter.get("/recentpost", async (c) => {
  const prisma = new import_edge2.PrismaClient({
    datasourceUrl: c.env?.DATABASE_URL
  }).$extends(withAccelerate());
  const blogs = await prisma.post.findMany({
